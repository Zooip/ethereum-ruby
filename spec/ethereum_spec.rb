...
describe Ethereum do
  ...
  before(:all) do
    @client = Ethereum::HttpClient.new(HTTP_CLIENT_ADDRESS,true)
    @formatter = Ethereum::Formatter.new
  end

  describe "Ethereum Version" do
    it 'has a version number' do
      expect(Ethereum::VERSION).to eq("1.5.4")
    end
  end
  
  describe "Deployment" do
    it "should deploy a contract with parameters" do
      @init = Ethereum::Initializer.new(File.join(FIXTURES_PATH,"/ContractWithParams.sol"), @client)
      @init.build_all
  ...
  describe "Basic contract testing" do
    ...
    before(:all) do
      @init = Ethereum::Initializer.new(File.join(FIXTURES_PATH,"/SimpleNameRegistry.sol"), @client)
      @init.build_all
